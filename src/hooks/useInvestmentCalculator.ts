...
type ReturnsPerInvestment = { investmentOptionId: string, returnsByYear: ReturnsByYear[] };
...
const useInvestmentCalculator = () => {
  ...
  const calculateInvestmentNeededForGoals = (plannerData: PlannerData): GoalWiseInvestmentSuggestions[] => {
  ...
  const calculateReturnOnInvestment = (monthlyInvestment: number, termInMonths: number, yearlyReturnPercentage: number) => {

    const i = yearlyReturnPercentage / 100 / 12;
    return monthlyInvestment * ((Math.pow(1 + i, termInMonths) - 1) / i) * (1 + i);
  };

  function calculateYearWiseReturnsForSuggestion(startYear: number, endYear: number, investmentSuggestion: InvestmentSuggestion) {
    const returnsByYear: ReturnsByYear[] = [];

    for (let i = startYear + 1; i <= endYear; i++) {
      const termInMonths = (i - startYear) * 12;
      const expectedReturnPercentage = investmentOptions.filter(e => e.id === investmentSuggestion.investmentOptionId)[0].expectedReturnPercentage;
      const returnOfTheYear = calculateReturnOnInvestment(investmentSuggestion.amount, termInMonths, expectedReturnPercentage);
  ...
  function calculateYearWiseReturnForGoal(goal: FinancialGoal, goalWiseSuggestion: GoalWiseInvestmentSuggestions) {
    ...
    goalWiseSuggestion.investmentSuggestions.forEach(investmentSuggestion => {
      const returnsByYear = calculateYearWiseReturnsForSuggestion(startYear, endYear, investmentSuggestion);
      returnsByInvestment.push({ investmentOptionId: investmentSuggestion.investmentOptionId, returnsByYear });
    });
    return returnsByInvestment;
  }

  const calculateYearlyReturnValueBySuggestions = (financialGoals: FinancialGoal[], goalWiseInvestmentSuggestions: GoalWiseInvestmentSuggestions[]): GoalWiseReturn[] => {
    const goalWiseReturns: GoalWiseReturn[] = [];
    goalWiseInvestmentSuggestions.forEach(goalWiseSuggestion => {
      const goal = financialGoals.filter(e => e.getGoalName() === goalWiseSuggestion.goalName)[0];
      const returnsPerInvestment = calculateYearWiseReturnForGoal(goal, goalWiseSuggestion);
      goalWiseReturns.push({ goalName: goal.getGoalName(), returnsPerInvestment });
    });

    return goalWiseReturns;
  };

  return { calculateInvestmentNeededForGoals, calculateYearlyReturnValueBySuggestions };
};
