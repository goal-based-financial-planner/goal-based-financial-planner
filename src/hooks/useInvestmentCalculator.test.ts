import useInvestmentCalculator, { GoalWiseInvestmentSuggestions } from './useInvestmentCalculator';
...
describe('Test useInvestmentCalculator', () => {
  ...
  it('should compute return for generated investment suggestions for goals', () => {
    ...
    const investmentSuggestions = calculateInvestmentNeededForGoals(plannerData);

    // Act
    const result = calculateYearlyReturnValueBySuggestions(plannerData.financialGoals, investmentSuggestions);

    function getReturnsByYears(resultIndex: number, investmentIndex: number) {
      return result[resultIndex].returnsPerInvestment[investmentIndex].returnsByYear;
    }
    ...
    expect(result[0].returnsPerInvestment[0].investmentOptionId).toEqual('assetType_1');
    expect(result[0].returnsPerInvestment[1].investmentOptionId).toEqual('assetType_2');
    ...
    expect((firstInvestmentReturns[firstInvestmentReturns.length - 1].return) + (secondInvestmentReturns[secondInvestmentReturns.length - 1].return)).toBeCloseTo(plannerData.financialGoals[0].getInflationAdjustedTargetAmount(), 0);

    expect(result[1].goalName).toEqual('goal2');
    expect(result[1].returnsPerInvestment.length).toEqual(2);
    expect(result[1].returnsPerInvestment[0].investmentOptionId).toEqual('assetType_2');
    expect(result[1].returnsPerInvestment[1].investmentOptionId).toEqual('assetType_3');
    ...
    expect((firstInvestmentReturnsGoal2[firstInvestmentReturnsGoal2.length - 1].return) + (secondInvestmentReturnsGoal2[secondInvestmentReturnsGoal2.length - 1].return)).toBeCloseTo(plannerData.financialGoals[1].getInflationAdjustedTargetAmount(), 0);

    expect(result[2].goalName).toEqual('goal3');
    expect(result[2].returnsPerInvestment.length).toEqual(1);
    expect(result[2].returnsPerInvestment[0].investmentOptionId).toEqual('assetType_3');
    const firstInvestmentReturnsGoal3 = getReturnsByYears(2, 0);
    expect(firstInvestmentReturnsGoal3.length).toEqual(2);
    expect((firstInvestmentReturnsGoal3[firstInvestmentReturnsGoal3.length - 1].return)).toBeCloseTo(plannerData.financialGoals[2].getInflationAdjustedTargetAmount(), 0);
